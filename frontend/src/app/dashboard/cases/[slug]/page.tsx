...
import { Users, Clock, Star, ShoppingCart, Package, ArrowLeft, Loader2 } from 'lucide-react'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="card p-5 space-y-4">
            ...
            {c.priceDigital && (
              <div className="space-y-2">
                ...
                <button onClick={() => handleAdd('digital')} className="btn-primary w-full">
                  Comprar Digital
                </button>
              </div>
            )}
            ...
            {c.pricePhysical && (
              <div className="space-y-2 pt-2 border-t border-crime-border">
                ...
                <button onClick={() => handleAdd('physical')} className="btn-secondary w-full">
                  Comprar Kit Físico
                </button>
              </div>
            )}
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {c.tags?.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-6">
                {c.tags.map((tag: string) => (
                  <span key={tag} className="badge bg-crime-muted/50 text-crime-text-faint text-[10px] font-mono">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          <div className="card p-6">
            <p className="text-xs font-mono uppercase tracking-widest text-crime-red mb-4">Sinopse</p>
            <p className="text-crime-text-secondary leading-relaxed text-sm whitespace-pre-line">
              {c.description}
            </p>
          </div>
          ...
          {c.previewImages?.length > 0 && (
            <div>
              <p className="text-xs font-mono uppercase tracking-widest text-crime-text-faint mb-3">Pré-visualização</p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {c.previewImages.map((url: string, i: number) => (
                  <div key={i} className="card overflow-hidden aspect-video">
